...
DEFAULT_PHASE_WEIGHTS = {
    # Heavier observability weight during survey to encourage 3–4 beacons
    "survey": (0.9, 0.05, 0.05),
    # Cruise stays balanced
    "cruise": (0.5, 0.3, 0.2),
    # Transit trades a bit more for energy
    "transit": (0.4, 0.5, 0.1),
    # Low power strongly prioritizes energy to drop to minimal beacons
    "low_power": (0.1, 0.8, 0.1),
}
...
def main():
    ...
    if args.mode == "manual":
        ...
        if args.policy_type == "weighted":
            ...
            if args.policy_type == "v2":
                ...
                dwell_steps = max(1, int(round(args.min_dwell_sec))) if args.min_dwell_sec > 0 else 1
    ...
    with open(out_dir / "selector_meta.json", "w") as f:
        json.dump(selector_meta_log, f, indent=2)
