...
class AdaptiveModemManagerV2:
    ...
    # ------------------------------------------------------------------
    def _update_uncert_gate(self, pos_metric: float) -> None:
        if not self.enable_uncertainty_gate:
            self._acoustics_enabled = True
            return
        if self._acoustics_enabled and pos_metric < self.gate_off_threshold:
            self._acoustics_enabled = False
        elif (not self._acoustics_enabled) and pos_metric > self.gate_on_threshold:
            self._acoustics_enabled = True
    ...
    def select(
        ...
        pos_metric = float(np.trace(Ppos)) / float(Ppos.shape[0])
        self._update_uncert_gate(pos_metric)
        ...
        # dwell hold (only when we already have a non-empty selection)
        if self._dwell < self.min_dwell_steps and self._current_ids and all(i in avail for i in self._current_ids):
            self._dwell += 1
